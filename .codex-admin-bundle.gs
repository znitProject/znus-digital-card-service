...
const ZNUS_BACKGROUND_KEYS = ['profile', 'role', 'contact', 'company', 'links'];
...
const FORM_BG_KEYS = ['profile','role','contact','company','links'];
...
const FORM_QUESTION_TITLES = {
  ...
  mobilePhone: '공개 휴대전화', publicEmail: '공개 이메일', profileImageFileId: '프로필 사진', profileBackgroundMode: '프로필 카드 배경', profileBackgroundFileId: '프로필 카드 배경 파일',
...
function setupFormAutomationForm() {
  ...
  form.addSectionHeaderItem().setTitle('사진과 배경 파일 업로드').setHelpText('프로필 사진은 필수입니다. 프로필을 포함한 각 카드 배경에는 JPG, PNG, WEBP 이미지 또는 MP4 영상을 사용할 수 있습니다. 배경 영상은 최대 5초까지 사용할 수 있습니다. 파일을 올리지 않으면 기존 배경을 유지합니다. 필요한 질문: ' + FORM_UPLOAD_KEYS.map(formQuestionTitle_).join(', '));
...
function onFormSubmitCard(e) {
  return withWorkspaceLock_(function () {
    ...
    try {
      ...
      next.profileImageFileId = formStoreAsset_(data.profileImageFileId, '', base.cardId, 'profile', 'IMAGE');
      FORM_BG_KEYS.forEach(function (key) {
...
function formValidate_(input, existing) {
  ...
  formInspectAsset_(out.profileImageFileId, 'IMAGE', '프로필 이미지');
  FORM_BG_KEYS.forEach(function (key) {
...
function getAdminPreviewMedia(token, section, useDefault) {
  ...
  if (!card) return null;
  const company = publicRows_(ss.getSheetByName('CompanySettings'))[0] || {};
  const defaultVideo = section !== 'logo' && (useDefault === true || card[section + 'BackgroundMode'] === 'DEFAULT' || !card[section + 'BackgroundMode']);
  if (section !== 'logo' && !defaultVideo && card[section + 'BackgroundMode'] !== 'VIDEO') return null;
  const fileId = section === 'logo' ? company.companyLogoFileId : defaultVideo ? company[section + 'DefaultVideoFileId'] : card[section + 'BackgroundFileId'];
...
function getEmployees() {
  ...
  return cards.map(card => ({
    ...
    backgrounds: ['profile', 'role', 'contact', 'company', 'links'].map(key => ({type: key, mode: text(card[key + 'BackgroundMode']) || 'DEFAULT'})),
