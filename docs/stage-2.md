...
1. `setupFormAutomationWorkspace`를 한 번 실행해 `Cards`, `CompanySettings`, `DeletedTokens` 헤더를 준비합니다.
2. `setupFormAutomationForm`을 한 번 실행해 질문, 파일 업로드, 응답 Sheet, 제출 트리거를 구성합니다.
