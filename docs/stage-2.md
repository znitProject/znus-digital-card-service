...
2. 마지막에 `setupFormAutomationAll`을 한 번 실행해, 필요한 Google 권한을 한 번에 승인하고 `Cards`, `CompanySettings`, `DeletedTokens`, 텍스트·배경 선택 Form 항목, 제출 트리거를 구성합니다.
3. Form 화면에서 파일 업로드 질문 5개를 수동으로 추가합니다. 질문 제목은 `프로필 사진`, `직무 카드 배경 파일`, `연락처 카드 배경 파일`, `회사 카드 배경 파일`, `링크 카드 배경 파일`을 사용합니다. `프로필 사진`은 필수 이미지 1개로, 나머지는 선택 이미지 또는 동영상 1개로 설정합니다. 응답자에게는 이미지/영상으로 변경을 선택했을 때만 파일을 올리도록 안내합니다.
