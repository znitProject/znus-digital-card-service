...
function requestOrigin(req) {
  ...
  return host ? `${requestUsesHttps(req) ? 'https' : 'http'}://${host}` : '';
}
...
function shareProfileImageUrl(assets, origin) {
  const profile = assets.find(asset => asset.slot === 'profile' && asset.kind === 'IMAGE');
  if (!profile) return '';
  const mime = String(profile.mime_type || '');
  if (mime && !['image/jpeg', 'image/png', 'image/webp'].includes(mime)) return '';
  if (useSupabaseStorage && profile.storage_key) return supabaseObjectUrl(profile.storage_key);
  return origin ? `${origin}/media/${profile.id}` : '';
}
...
async function publicCardData(token) {
  ...
  const assets = (await pool.query(`
    SELECT a.id, b.page::text AS slot, a.kind, a.mime_type, a.storage_key, b.crop_x, b.crop_y, b.crop_scale
    FROM card_page_backgrounds b JOIN media_assets a ON a.id=b.media_asset_id
    WHERE b.employee_id=$1
    UNION ALL
    SELECT a.id, d.page::text AS slot, a.kind, a.mime_type, a.storage_key, d.crop_x, d.crop_y, d.crop_scale
    ...
    UNION ALL
    SELECT id, slot, kind, mime_type, storage_key, 50::double precision AS crop_x, 50::double precision AS crop_y, 1::double precision AS crop_scale
    FROM (
      SELECT DISTINCT ON (slot) id, slot::text AS slot, kind, mime_type, storage_key
      FROM media_assets
...
function renderCardHtml(data, origin = '') {
...
async function route(req, res) {
  ...
  if (req.method === 'GET' && /^\/[A-Za-z0-9_-]{12,30}$/.test(url.pathname)) {
    ...
    return data ? html(res, 200, renderCardHtml(data, requestOrigin(req))) : html(res, 404, '<h1>이용할 수 없는 명함입니다.</h1>');
  }
